...
2. Generate two CSV files, `local-authorities.csv` and `consortia.csv`.
   - See `local-authorities.csv.example` and `consortia.csv.example` for examples of how we expect the data to be structured. These are for reference only, you will export the live data in the following steps.
   - For information of where to source this information, see our internal documentation page headered *DESNZ Support: Onboarding LAs*.
     - `local-authorities.csv` can be generated by exporting *DESNZ SST* to a csv. Make sure that you export the 'WHLG' page of the spreadsheet and not any other page.
