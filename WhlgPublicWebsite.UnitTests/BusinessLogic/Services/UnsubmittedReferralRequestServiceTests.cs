...
public class UnsubmittedReferralRequestsServiceTests
{
    ...
    public async Task WriteUnsubmittedReferralRequestsToCsv_WhenCalledWithNewReferral_UpdatesReferralCreated()
    {
        ...
        mockDataAccessProvider.Setup(dap => dap.GetWhlgUnsubmittedReferralRequestsAsync().Result)
            .Returns(newReferralList);
        mockDataAccessProvider.Setup(dap =>
                dap.GetWhlgReferralRequestsByCustodianAndRequestDateAsync(newReferralList[0].CustodianCode, 3,
                    2023).Result)
            .Returns(newReferralList);
        ...
        WriteUnsubmittedReferralRequestsToCsv_WhenCalledWithMultipleNewReferralsForDifferentCustodianCodes_CreatesMultipleFiles()
    {
        ...
        mockDataAccessProvider.Setup(dap => dap.GetWhlgUnsubmittedReferralRequestsAsync().Result)
            .Returns(newReferralList);
        mockDataAccessProvider.Setup(dap =>
                dap.GetWhlgReferralRequestsByCustodianAndRequestDateAsync("5", 3, 2023).Result)
            .Returns(allReferralListForCustodianCode5);
        mockDataAccessProvider.Setup(dap =>
                dap.GetWhlgReferralRequestsByCustodianAndRequestDateAsync("6", 3, 2023).Result)
